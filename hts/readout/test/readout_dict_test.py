...
def test_create_from_envision_csv(path):
    ...
    test_plate = readout_dict.ReadoutDict.create(path = os.path.join(path, TEST_FILE_ENVISION), format = "envision_csv")
...
def test_create_from_insulin_csv(path_insulin):
    ...
    test_plate = readout_dict.ReadoutDict.create(path = os.path.join(path_insulin, TEST_FILE_INSULIN), format = "insulin_csv")
