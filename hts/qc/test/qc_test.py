...
TEST_RUN = {(0,0): TEST_PLATE, (2,0): TEST_PLATE2}
TEST_RUN = {i: np.array([np.array([float(read) for read in column]) for column in plate]) for i, plate in TEST_RUN.items()}
TEST_PLATE_NAME = "test_plate_name"
...
def test_create_basic_heatmap(path):
    test_readout = plate.Plate(TEST_RUN, name=TEST_PLATE_NAME)
